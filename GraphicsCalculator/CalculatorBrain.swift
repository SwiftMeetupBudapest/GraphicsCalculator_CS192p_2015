        ...
        set {
            if let opSymbols = newValue as? Array<String> {
                ...
                for opSymbol in opSymbols {
                    ...
                    } else if let operand = NSNumberFormatter().numberFromString(opSymbol.stringByReplacingOccurrencesOfString(".", withString: decimalSeparator, options: NSStringCompareOptions.LiteralSearch, range: nil))?.doubleValue {
                        newOpStack.append(.Operand(operand))
                    } else if opSymbol == "M" {
                        // Had to add variable loading to stack
                        newOpStack.append(Op.Variable(
                            opSymbol,
